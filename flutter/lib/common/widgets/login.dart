...
import 'package:gamedesk/common/hbbs/hbbs.dart';
...
class _OidcProviderBranding {
  ...
  const _OidcProviderBranding({required this.label, required this.iconKey});
}

_OidcProviderBranding _oidcProviderBranding(String op) {
  switch (op.toLowerCase()) {
    case 'azure':
      return _OidcProviderBranding(label: 'Microsoft', iconKey: 'microsoft');
    default:
      return _OidcProviderBranding(
        label:
            {'github': 'GitHub', 'gitlab': 'GitLab'}[op.toLowerCase()] ??
            toCapitalized(op),
...
class _IconOP extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final svgFile = kOpSvgList.contains(op.toLowerCase())
        ? op.toLowerCase()
        : 'default';
    return Container(
      margin: margin,
      child: icon == null
          ? SvgPicture.asset('assets/auth-$svgFile.svg', width: 20)
          : SvgPicture.string(icon!, width: 20),
    );
  }
}
        ...
        Container(
          ...
          child: Obx(
            () => ElevatedButton(
              ...
              onPressed: curOP.value == 'rustdesk' || !canStartAuth()
                  ? null
                  : onTap,
              child: Row(
                children: [
  ...
  Future<void> _updateState(int authAttempt) {
    ...
    return bind
        ...
        .then<void>((result) {
          ...
          final stateMsg =
              backendStateMsg == _requestingAccountAuth &&
                  (url == null || url.isEmpty)
              ? _waitingAccountAuth
              : backendStateMsg;
          final bool urlLaunched =
              (resultMap['url_launched'] as bool?) ?? false;
          ...
          final newUrl = _url.isEmpty && url != null && url.isNotEmpty
              ? url
              : null;
                ...
                Container(
                  ...
                  child: Obx(
                    () => ElevatedButton(
                      ...
                      onPressed:
                          (curOP.value.isEmpty || curOP.value == 'gamedesk') &&
                              !isInProgress
                          ? () {
                              onLogin();
                            }
              ...
              child: InkWell(
                child: Icon(
                  ...
                  color: isCloseHovered
                      ...
                      : Theme.of(
                          context,
                        ).textTheme.titleLarge?.color?.withOpacity(0.55),
                ),
