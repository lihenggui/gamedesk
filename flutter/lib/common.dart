...
import 'package:gamedesk/utils/platform_channel.dart';
...
import 'package:gamedesk/native/win32.dart'
    if (dart.library.html) 'package:gamedesk/web/win32.dart';
import 'package:gamedesk/native/common.dart'
    if (dart.library.html) 'package:gamedesk/web/common.dart';
...
Future<bool> restoreWindowPosition(WindowType type,
  ...
  if (bind
      .mainGetEnv(key: "DISABLE_GAMEDESK_RESTORE_WINDOW_POSITION")
      .isNotEmpty) {
    return false;
  }
  if (type != WindowType.Main && windowId == null) {
...
List<String>? urlLinkToCmdArgs(Uri uri) {
  ...
  if (uri.authority.isEmpty &&
    ...
    // gamedesk://<connect-id>
    // gamedesk://<connect-id>/r
    // gamedesk://<connect-id>/r@<server>
    command = '--connect';
    id = uri.authority;
    if (uri.path.length > 1) {
      id = id + uri.path;
...
Future<void> onActiveWindowChanged() async {
  ...
  if (rustDeskWinManager.getActiveWindows().isEmpty) {
    ...
    try {
      ...
      debugPrint("Start closing GameDesk...");
...
const kMacOSPermChannel = MethodChannel("me.blocker.gamedesk/host");
