...
// The bundle keeps the core library at lib/libgamedesk.so next to the
...
static void* dlopen_bundled_lib() {
  ...
  *last_slash = '\0';
  ...
  void* libgamedesk = dlopen(lib_path, RTLD_LAZY);
  if (!libgamedesk) {
    char* error = dlerror();
    if (error != nullptr) {
...
bool flutter_rustdesk_core_main() {
   void* libgamedesk = dlopen_bundled_lib();
   if (!libgamedesk) {
      libgamedesk = dlopen(RUSTDESK_LIB_PATH, RTLD_LAZY);
   }
   if (!libgamedesk) {
