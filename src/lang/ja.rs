...
pub static ref T: std::collections::HashMap<&'static str, &'static str> =
    [
        ...
        ("Ready", "準備完了"),
        ("Established", "接続完了"),
        ("connecting_status", "GameDesk ネットワークに接続中..."),
        ...
        ("install_tip", "UAC の影響により、GameDesk がリモートデスクトップ上で正常に動作しない場合があります。UAC を回避するには、下のボタンをクリックしてシステムに GameDesk をインストールしてください。"),
        ("Click to upgrade", "アップグレード"),
        ("Configure", "設定"),
        ("config_acc", "リモートからあなたのコンピューターを操作するには、GameDesk に「アクセシビリティ」権限を与える必要があります。"),
        ("config_screen", "リモートからあなたのコンピューターにアクセスするには、GameDesk に「画面録画」の権限を与える必要があります。"),
        ("Installing ...", "インストール中..."),
        ("Install", "インストール"),
        ("Installation", "インストール"),
        ...
        ("Discovered", "発見済み"),
        ("install_daemon_tip", "起動時に GameDesk を開始するには、システムサービスをインストールする必要があります。"),
        ("Remote ID", "リモート ID"),
        ("Paste", "貼り付け"),
        ...
        ("android_input_permission_tip1", "この Android デバイスをリモートコンピューターからマウスやタッチで操作するには、GameDesk に「ユーザー補助」からサービスの使用を許可する必要があります。"),
        ("android_input_permission_tip2", "次の端末設定ページに進み、「インストール済みアプリ」から「GameDesk Input」を有効にしてください。"),
        ...
        ("Turned off", "オフになりました"),
        ("Language", "言語"),
        ("Keep GameDesk background service", "GameDesk バックグラウンドサービスを維持"),
        ("Ignore Battery Optimizations", "バッテリーの最適化を無効にする"),
        ("android_open_battery_optimizations_tip", "この機能を使わない場合は、GameDesk アプリの設定ページから「バッテリー」に進み、「制限しない」を選択してください。"),
        ...
        ("elevated_foreground_window_tip", "リモートデスクトップでフォーカスされているウィンドウの操作にはより高い権限が必要なため、マウスとキーボードが一時的に使用できなくなっています。リモートユーザーにウィンドウを最小化、または接続管理画面から権限を昇格するよう要求してください。この問題を回避するには、リモートコンピューターに GameDesk をインストールしてください。"),
        ("Disconnected", "切断しました"),
        ("Other", "その他"),
        ...
        ("software_render_tip", "Linux で NVIDIA 製のグラフィックカードを使用していると、接続後すぐにリモートウィンドウが閉じてしまう場合があります。オープンソースの Nouveau ドライバーに切り替えて、ソフトウェアレンダリングを使用するよう設定すると解決するかもしれません。(GameDesk の再起動が必要です)"),
        ("Always use software rendering", "常にソフトウェアレンダリングを使用する"),
        ("config_input", "リモートコンピューターをキーボードで操作するには、GameDesk に「入力監視」権限を与える必要があります。"),
        ("config_microphone", "リモートコンピューターと通話するには、GameDesk に「音声録音」権限を与える必要があります。"),
        ...
        ("still_click_uac_tip", "リモートデスクトップユーザーが GameDesk を実行する際に、UACを許可する必要があります。"),
        ...
        ("View Mode", "表示モード"),
        ("verify_rustdesk_password_tip", "GameDesk のパスワードを確認する"),
        ...
        ("scam_text1", "もし、知らない相手から電話で GameDesk のインストールやサービスの開始を依頼された場合、作業を進めずに、すぐに電話を切ってください。"),
        ...
        ("upgrade_rustdesk_server_pro_to_{}_tip", "GameDesk Server Pro をバージョン {} 以上にアップグレードしてください！"),
        ...
        ("selinux_tip", "SELinuxが有効になっているため、GameDesk が正常に動作しない可能性があります。"),
        ("Change view", "表示を変更"),
        ("Big tiles", "大きなタイル"),
        ("Small tiles", "小さなタイル"),
        ("List", "リスト"),
        ...
        ("powered_by_me", "Powered by GameDesk"),
        ("outgoing_only_desk_tip", "カスタマイズされたエディションを使用しています。\n他のコンピューターに接続できますが、他のコンピューターからのリクエストは受信できません。"),
        ("preset_password_warning", "このエディションには、既定で固定パスワードが設定されています。このパスワードを知っているユーザーはあなたのデバイスを完全にコントロールできるため、そのような危険がある場合は直ちに GameDesk をアンインストールして下さい！"),
        ...
        ("allow-only-conn-window-open-tip", "GameDesk のウィンドウが開いている場合のみ接続を許可する"),
        ...
        ("floating_window_tip", "GameDesk のバックグラウンドサービスを維持するために使用されます。"),
        ...
        ("upgrade_remote_rustdesk_client_to_{}_tip", "リモート側の GameDesk クライアントをバージョン {} 以上にアップグレードしてください！"),
        ...
        ("allow-insecure-tls-fallback-tip", "既定では GameDesk は TLS を使用するプロトコルのサーバー証明書を検証します。\nこのオプションを有効化すると GameDesk は検証の手順をスキップして、検証に失敗した場合の処理を続行します。"),
        ("Disable UDP", "UDP を無効化する"),
        ("disable-udp-tip", "TCP のみ使用するかどうかを制御します。\nこのオプションを有効化すると、GameDesk は UDP 20116 を使用せずに TCP 20116 を使用するようになります。"),
        ("server-oss-not-support-tip", "注意: GameDesk Server OSS にはこの機能が含まれていません。"),
        ...
        ("GameDesk cannot reach the desktop session on the remote device, check that a desktop session is running and that GameDesk can use it", "GameDesk はリモート端末のデスクトップセッションにアクセスできません。セッションが動作していて GameDesk から利用できることを確認してください"),
        ...
        ("GameDesk could not obtain a usable screen from the XDG Desktop Portal, the PipeWire library may be too old", "GameDesk は XDG Desktop Portal から使用可能な画面を取得できませんでした。PipeWire ライブラリが古すぎる可能性があります"),
        ("GameDesk could not load a GStreamer component needed for screen capture ({})", "GameDesk は画面キャプチャに必要な GStreamer コンポーネントを読み込めませんでした ({})"),
