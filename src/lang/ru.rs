...
pub static ref T: std::collections::HashMap<&'static str, &'static str> =
    [
        ...
        ("connecting_status", "Подключение к сети GameDesk..."),
        ...
        ("install_tip", "В некоторых случаях из-за UAC GameDesk может работать неправильно на удалённом узле. Чтобы избежать возможных проблем с UAC, нажмите кнопку ниже для установки GameDesk в системе."),
        ("Click to upgrade", "Нажмите, чтобы обновить"),
        ("Configure", "Настроить"),
        ("config_acc", "Чтобы удалённо управлять своим рабочим столом, вы должны предоставить GameDesk права \"доступа\""),
        ("config_screen", "Для удалённого доступа к рабочему столу вы должны предоставить GameDesk права \"снимок экрана\""),
        ...
        ("android_input_permission_tip1", "Чтобы удалённое устройство могло управлять вашим Android-устройством с помощью мыши или нажатий, необходимо разрешить GameDesk использовать службу \"Специальные возможности\"."),
        ("android_input_permission_tip2", "Перейдите на соответствующую страницу системных настроек, найдите и войдите в \"Установленные службы\", включите службу \"GameDesk Input\"."),
        ...
        ("Turned off", "Отключён"),
        ("Language", "Язык"),
        ("Keep GameDesk background service", "Держать в фоне службу GameDesk"),
        ...
        ("Show GameDesk", "Показать GameDesk"),
        ...
        ("config_input", "Чтобы управлять удалённым рабочим столом с помощью клавиатуры, необходимо предоставить GameDesk разрешения \"Мониторинг ввода\"."),
        ("config_microphone", "Чтобы разговаривать с удалённой стороной, необходимо предоставить GameDesk разрешение \"Запись аудио\"."),
        ...
        ("still_click_uac_tip", "По-прежнему требуется, чтобы удалённый пользователь нажал \"OK\" в окне UAC при запуске GameDesk."),
        ...
        ("verify_rustdesk_password_tip", "Подтвердить пароль GameDesk"),
        ...
        ("scam_text1", "Если вы разговариваете по телефону с кем-то, кого вы НЕ ЗНАЕТЕ и НЕ ДОВЕРЯЕТЕ, и он просит вас использовать GameDesk и запустить его службу, не продолжайте и немедленно прервите разговор."),
        ...
        ("upgrade_rustdesk_server_pro_to_{}_tip", "Обновите GameDesk Server Pro до версии {} или новее!"),
        ...
        ("selinux_tip", "На вашем устройстве включён SELinux, что может помешать правильной работе GameDesk на управляемой стороне."),
        ("Change view", "Вид"),
        ...
        ("powered_by_me", "Основано на GameDesk"),
        ...
        ("allow-only-conn-window-open-tip", "Разрешать подключение только при открытом окне GameDesk"),
        ...
        ("floating_window_tip", "Помогает поддерживать фоновую службу GameDesk"),
        ...
        ("upgrade_remote_rustdesk_client_to_{}_tip", "Обновите клиент GameDesk до версии {} или новее на удалённой стороне!"),
        ...
        ("allow-insecure-tls-fallback-tip", "По умолчанию GameDesk проверяет сертификат сервера на наличие протоколов, использующих TLS.\nЕсли эта функция включена, GameDesk пропустит данный этап и продолжит работу в случае неудачной проверки."),
        ("Disable UDP", "Отключить UDP"),
        ("disable-udp-tip", "Определяет, следует ли использовать только TCP.\nЕсли включено, GameDesk не будет использовать UDP 20116, вместо него будет использоваться TCP 20116."),
        ("server-oss-not-support-tip", "ПРИМЕЧАНИЕ: в OSS-сервере GameDesk эта функция отсутствует."),
        ...
        ("GameDesk cannot reach the desktop session on the remote device, check that a desktop session is running and that GameDesk can use it", "GameDesk не может получить доступ к сеансу рабочего стола на удалённом устройстве, проверьте, что сеанс запущен и доступен GameDesk"),
        ...
        ("GameDesk could not obtain a usable screen from the XDG Desktop Portal, the PipeWire library may be too old", "GameDesk не смог получить пригодный экран от XDG Desktop Portal, библиотека PipeWire может быть слишком старой"),
        ("GameDesk could not load a GStreamer component needed for screen capture ({})", "GameDesk не удалось загрузить компонент GStreamer, необходимый для захвата экрана ({})"),
