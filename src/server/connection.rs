        ...
        if let Some(message::Union::LoginRequest(lr)) = msg.union {
            ...
            match msg.union {
                ...
                Some(message::Union::MouseEvent(mut me)) => {
                    ...
                    #[cfg(windows)]
                    if let Some(hwnd) =
                        video_service::window_capture::get_window_hwnd(self.display_idx)
                    {
                        ...
                        if evt_type != MOUSE_TYPE_MOVE_RELATIVE {
                            if let Some((sx, sy)) =
                                crate::platform::windows::window_client_to_screen(hwnd, me.x, me.y)
                            {
                                me.x = sx;
                                me.y = sy;
                            }
                ...
                window_capture::ScannerEvent::ViewedWindowClosed(closed_idx) => {
                    if self.display_idx == closed_idx {
                        log::info!(
                            ...
                            closed_idx,
                            self.monitor_display_idx
                        );
                        if let Some(server) = self.server.upgrade() {
                            let old_service_name =
                                window_capture::get_window_service_name(closed_idx);
                            {
                                let mut lock = server.write().unwrap();
                                lock.subscribe(&old_service_name, self.inner.clone(), false);
                                // Re-subscribe to the monitor service
                                let monitor_service = video_service::get_service_name(
                                    self.video_source(),
                                    self.monitor_display_idx,
                                );
