...
hbb_name = 'gamedesk' + ('.exe' if windows else '')
...
def measured_glibc_floor():
    ...
    paths = [p for p in glob.glob('tmpdeb/usr/lib/rustdesk/libdrmtap.so.0.*')
             + glob.glob('tmpdeb/usr/share/rustdesk/lib/libgamedesk.so')
...
def _carries_drmtap_marker(path, marker=DRMTAP_DLOPEN_MARKER):
    ...
    # libgamedesk.so is ~45 MB and there is no reason to hold it all in memory, and the `with`
...
def assert_staged_binary_is_drm():
    ...
    """
    binaries = [p for p in glob.glob('tmpdeb/usr/share/rustdesk/lib/libgamedesk.so')
...
def build_flutter_dmg(version, features):
    ...
    os.rename("rustdesk.dmg", f"../gamedesk-{version}.dmg")
    '''
    os.chdir("..")
...
def build_flutter_windows(version, features, skip_portable_pack):
    ...
    system2(
        f'python3 ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/gamedesk.exe')
    os.chdir('../..')
    if os.path.exists('./gamedesk_portable.exe'):
        os.replace('./target/release/gamedesk-portable-packer.exe',
                   './gamedesk_portable.exe')
    else:
        os.rename('./target/release/gamedesk-portable-packer.exe',
                  './gamedesk_portable.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/gamedesk_portable.exe')
    os.rename('./gamedesk_portable.exe', f'./gamedesk-{version}-install.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/gamedesk-{version}-install.exe')
...
def main():
    ...
    if windows:
        ...
        # system2('upx.exe target/release/gamedesk.exe')
        system2('mv target/release/gamedesk.exe target/release/GameDesk.exe')
        pa = os.environ.get('P')
        if pa:
            # https://certera.com/kb/tutorial-guide-for-safenet-authentication-client-for-code-signing/
            system2(
                f'signtool sign /a /v /p {pa} /debug /f .\\cert.pfx /t http://timestamp.digicert.com  '
                'target\\release\\GameDesk.exe')
        else:
            print('Not signed')
        os.makedirs(res_dir, exist_ok=True)
        system2(
            f'cp -rf target/release/GameDesk.exe {res_dir}')
        os.chdir('libs/portable')
        system2('pip3 install -r requirements.txt')
        system2(
            f'python3 ./generate.py -f ../../{res_dir} -o . -e ../../{res_dir}/gamedesk-{version}-win7-install.exe')
        system2(f'mv ../../{res_dir}/gamedesk-{version}-win7-install.exe ../..')
